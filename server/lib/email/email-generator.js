 ...
 * @returns {string} html for an email containing info about a test which is due for a patient
 */
function passwordRecoveryEmail(email_info) {
  const header_image_url =
    "https://images.unsplash.com/photo-1528872042734-8f50f9d3c59b";

  const username = email_info.user.username;
          ...
          <mj-section padding-top="30px">
             <mj-column width="100%">
                <mj-text>
                   ...
                   <p>This is an autogenerated email with your new password</p>
                   <p>Please login using your username and this new password: <b>${new_password}</b></p>
                   <p>You can change your password once you are logged in.</b></p>
                </mj-text>
             </mj-column>
          </mj-section>
       </mj-body>
  ...
  if (computed_html.errors.length === 0) {
    const email = email_info.user.recovery_email;
    return { "to": email, "html": computed_html.html };
  }
  return null;
}
