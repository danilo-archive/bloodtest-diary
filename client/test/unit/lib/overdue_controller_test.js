...
describe("Grouping", function (){
    it("shoud correctly group the tests in classes", function (){
        var tests = [{due_date: new Date(2019, 0, 3)}, {due_date: new Date(2019, 1, 1)},{due_date: new Date(2019, 2, 1)},
                     {due_date: new Date(2019, 0, 1)}, {due_date: new Date(2017, 11, 1)},{due_date: new Date(2019, 1, 20)},
                     {due_date: new Date(2019, 1, 13)}, {due_date: new Date(2019, 1, 4)}, {due_date: new Date(2019, 0, 15)}];
        let groups = overdue_controller.testGroup(tests);

        groups[0].class.should.equal("Year+");
        groups[0].tests.length.should.equal(1);
            groups[0].tests[0].due_date.getDate().should.equal(1);
            groups[0].tests[0].due_date.getMonth().should.equal(11);
            groups[0].tests[0].due_date.getFullYear().should.equal(2017);
        ...
        groups[2].tests.length.should.equal(3);
            groups[2].tests[0].due_date.getDate().should.equal(1);
            groups[2].tests[0].due_date.getMonth().should.equal(0);
            groups[2].tests[0].due_date.getFullYear().should.equal(2019);
            groups[2].tests[1].due_date.getDate().should.equal(3);
            groups[2].tests[1].due_date.getMonth().should.equal(0);
            groups[2].tests[1].due_date.getFullYear().should.equal(2019);
            groups[2].tests[2].due_date.getDate().should.equal(15);
            groups[2].tests[2].due_date.getMonth().should.equal(0);
            groups[2].tests[2].due_date.getFullYear().should.equal(2019);

        groups[3].class.should.equal("2-4 weeks");
        groups[3].tests.length.should.equal(3);
            groups[3].tests[0].due_date.getDate().should.equal(1);
            groups[3].tests[0].due_date.getMonth().should.equal(1);
            groups[3].tests[0].due_date.getFullYear().should.equal(2019);
            groups[3].tests[1].due_date.getDate().should.equal(4);
            groups[3].tests[1].due_date.getMonth().should.equal(1);
            groups[3].tests[1].due_date.getFullYear().should.equal(2019);
            groups[3].tests[2].due_date.getDate().should.equal(13);
            groups[3].tests[2].due_date.getMonth().should.equal(1);
            groups[3].tests[2].due_date.getFullYear().should.equal(2019);

        groups[4].class.should.equal("Less than 2 weeks");
        groups[4].tests.length.should.equal(2);
            groups[4].tests[0].due_date.getDate().should.equal(20);
            groups[4].tests[0].due_date.getMonth().should.equal(1);
            groups[4].tests[0].due_date.getFullYear().should.equal(2019);
            groups[4].tests[1].due_date.getDate().should.equal(1);
            groups[4].tests[1].due_date.getMonth().should.equal(2);
            groups[4].tests[1].due_date.getFullYear().should.equal(2019);
    });
});
